...
int main(int argc, char *argv[]) {
    ...
    paper. When running the constraint_analysis executable:
        ./constraint_analysis.exe [map name] [scenario number] [number of agents] [algorithm]
    ...
    */
    ...
    if (argc != 6) {
        ...
        std::cout << "\t./constraint_analysis.exe [map name] [resolution] [scenario number] [number of agents] [algorithm]" << std::endl;
    ...
    size_t resolution = std::stoull(argv[2]);
    size_t scenarioNum = std::stoull(argv[3]);
    size_t numAgents = std::stoull(argv[4]);
    std::string mapf = argv[5];


    // create problem instance
    Problem* problemInstance = new Problem(mapName, scenarioNum, resolution);
