...
class PBS : public MAPF {
    ...
    public:
        ...
        using ConstraintMap        = std::map<size_t, std::vector<PriorityConstraint>>;                  // alias for a map that tracks agents and their applied priority constraints
    ...
    public:
        ...
        PBS(LowLevelFunction& _lowlevel, std::string _cost = "soc", size_t _resolution = 1, bool _debug = false) : 
            m_lowlevel(_lowlevel), 
    ...
    private:
        ...
        size_t FindMinimumEndTime(const Coord& _goal, const std::vector<SpatialMotionConstraint>& _constraints);
